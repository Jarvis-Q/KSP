...
module.exports = {
    ...
    /**
     * Detect file charset encoding, if no encoding matches, return undefined.
     * @param filename
     * @return {*}
     */
    ...
    /**
     * Covert file charset encoding.
     * @param filename
     * @param originCharset
     * @param targetCharset
     */
    fileCharsetConvert: function( filename, originCharset, targetCharset ){

        try {
            var originData = iconv.decode( Fs.readFileSync( filename ), originCharset );
            Fs.writeFileSync( filename, iconv.encode( originData, targetCharset ) );
        }
        catch( e ){
            Log.error( 'Charset convert fail, err:', JSON.stringify( e ) );
            process.exit( 1 );
        }
